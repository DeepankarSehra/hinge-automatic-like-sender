...
def combine_screenshots(folder, output_image, crop_height):
    ...
    screenshots.sort()  
    # if screenshots[0] == "masti.png":                                                           # if there already exists a joined lomg screenshot (masti.png)
    #     os.remove('screenshots/masti.png')

    images = []
    for i, img in enumerate(screenshots):
        image = Image.open(os.path.join(folder, img))
        if i != 0:                                                                              # to remove the namne that comes after every scroll
...
combine_screenshots("screenshots", "screenshots/masti.png", crop_height=123)                    # name on the profile stretches over 123 pixels from the top